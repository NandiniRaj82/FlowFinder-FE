...
const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  const handleChoiceSelect = async (choice: 'suggestions' | 'full-correction') => {
    ...
    try {
      ...
      if (msg.includes('FILE_TOO_LARGE')) msg = 'File too large. Max 200MB per file.';
      else if (msg.includes('INVALID_FILE_TYPE')) msg = 'Invalid file type.';
      else if (msg.includes('Not authenticated')) { msg = 'Session expired.'; setTimeout(() => { localStorage.clear(); window.location.href = '/signin'; }, 2000); }
      alert(`❌ Error: ${msg}`);
  ...
  const handleMatchSubmit = async (websiteUrl: string, figmaUrl: string) => {
    ...
    try {
      ...
      let msg = error.message;
      if (msg.includes('Not authenticated')) { msg = 'Session expired.'; setTimeout(() => { localStorage.clear(); window.location.href = '/signin'; }, 2000); }
      alert(`❌ Error: ${msg}`);
    ...
    <>
      ...
      {feature !== null && (
        <div className="min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-rose-50 relative overflow-hidden flex flex-col">
          ...
          {/* Loading overlay */}
          {(isProcessing || matchProcessing || redesignerProcessing) && (
              ...
              <div className="bg-white rounded-2xl p-8 shadow-2xl text-center max-w-md">
                ...
                <p className="text-sm text-slate-600">
                  {feature === 'match-design' ? 'Screenshotting site, fetching Figma, running comparison. ~30s.' :
                   feature === 'website-redesigner' ? 'Generating 3 full HTML redesigns. Please wait ~60s.' :
            ...
            <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <h1 className="text-2xl font-black bg-gradient-to-r from-orange-600 to-amber-700 bg-clip-text text-transparent"
                  style={{ fontFamily: '"Playfair Display", serif' }}>Flow Finder</h1>
              </div>
              ...
              <>
                      ...
                      {totalFiles <= 5 ? (
                        ...
                        <div className="text-sm text-slate-700">
                          ...
                          <details className="mt-2">
                            <summary className="cursor-pointer text-orange-600 font-medium">View all files</summary>
                ...
                {stage === 'chat' && (
                  <div className="animate-fade-in h-full">
                    <AccessibilityChat errors={apiResult} fileName={displayFileName} initialChoice={selectedChoice!} onReset={handleAccessibilityReset} />
                  </div>
                )}
              </>
            )}
